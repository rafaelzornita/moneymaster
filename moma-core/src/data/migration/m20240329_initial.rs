...
use crate::domain::{categories::category_defaults, CategoryEntity, EntryEntity, SettingEntity};
use sea_orm::{ConnectionTrait, EntityTrait, IntoActiveModel, Schema};
...
impl<'a> MyMigration<'a> for Migration {
    ...
    async fn create_tables(manager: &'a SchemaManager) {
        Self::create_table(manager, CategoryEntity).await;
        Self::create_table(manager, EntryEntity).await;
        Self::create_table(manager, SettingEntity).await;
    }
